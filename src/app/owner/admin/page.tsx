...
export default async function AdminPage() {
  ...
  const [flags, users] = await Promise.all([
    ...
    prisma.user.findMany({
      ...
      select: { id: true, supabase_id: true, email: true, name: true, role: true, status: true, created_at: true },
    }),
  ])
  ...
  return <AdminClient flags={flags.length > 0 ? (flags as any) : defaultFlags} initialUsers={users as any} />
}
